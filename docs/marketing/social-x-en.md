# X / Twitter posts (English)

Replace `{EN_LISTING}` before posting. One block = one tweet.

---

## A. Single tweet (general)

```
I bundled 80 boring-but-essential dev utilities into ONE API 🧰

QR & barcodes, invoices, Markdown→HTML, hashing, JWT, UUID, email/IBAN/card
validation, i18n + a Japanese text pack.

No AI = fast, cheap, deterministic. Free tier 👇
{EN_LISTING}

#buildinpublic #API #webdev
```

---

## B. Pain-point hook

```
Stop adding a new dependency for every tiny task.

Need a QR code? Hash? UUID? Invoice? IBAN check? JWT?
One API, one key:

curl -X POST ".../v1/qr" -H "X-RapidAPI-Key: KEY" \
  -d '{"text":"https://example.com","format":"svg"}'

80 endpoints. Free tier 👇
{EN_LISTING}
```

---

## C. Thread (5 tweets)

```
1/ Tired of pulling a library for every little job (QR, hashing, invoices,
JWT, validation…)? I put 80 of them behind a single API. No AI, so it's
fast + deterministic. 🧰
{EN_LISTING}
```
```
2/ Auth is one header. QR code:

curl -X POST ".../v1/qr" \
 -H "X-RapidAPI-Key: KEY" \
 -d '{"text":"https://example.com","format":"svg"}'

PNG or SVG. Wi-Fi-join and vCard QR too.
```
```
3/ Validators you actually reach for:
• email
• credit card (Luhn)
• IBAN (mod-97)
• phone (E.164)
• password strength
+ PII redaction. All one call away.
```
```
4/ Bonus niche pack: Japanese text 🇯🇵
full-width⇄half-width, kana, kana→romaji.
{"text":"とうきょう","operation":"romaji"} → "toukyou"
Rarely available elsewhere.
```
```
5/ Free tier to start, cheap usage-based pricing after (no inference cost).
Give it a spin 👇 and tell me what endpoint to add next.
{EN_LISTING}
```
